      ...
      <form onSubmit={handleSubmit}>
        <div className="p-6 space-y-4">
            <div>
                ...
                <select value={supplierId} onChange={e => setSupplierId(e.target.value)} required className="w-full p-2 border border-gray-300 rounded-md">
            ...
            <div>
             ...
             <div>
                ...
                <input type="text" value={description} onChange={e => setDescription(e.target.value)} required className="w-full p-2 border border-gray-300 rounded-md" placeholder="e.g., 5 bags of feed, Payment" />
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Amount ($)</label>
                <input type="number" step="0.01" value={amount} onChange={e => setAmount(Number(e.target.value))} required className="w-full p-2 border border-gray-300 rounded-md" />
        ...
        <div className="bg-gray-50 px-6 py-3 flex justify-end space-x-3 rounded-b-lg">
          ...
          <button type="submit" className="px-4 py-2 bg-brand-green-600 text-white font-semibold rounded-lg shadow-md hover:bg-brand-green-700">Save Transaction</button>
        </div>
      </form>
    </Modal>
  );
