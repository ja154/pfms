        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6">
            <div className="mb-4">
              ...
              <select value={recordType} onChange={e => setRecordType(e.target.value as RecordType)} disabled={isEditMode} className={`${inputStyles} disabled:bg-slate-100 disabled:cursor-not-allowed`}>
            ...
            {recordType === RecordType.FeedPurchase && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">Amount (kg)</label>
                    <input type="number" value={amount} onChange={e => setAmount(Number(e.target.value))} required min="0" className={inputStyles} />
                   </div>
                   <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">Total Cost ($)</label>
            ...
            {recordType === RecordType.Vaccination && (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Vaccine Type</label>
                  <input type="text" value={vaccineType} onChange={e => setVaccineType(e.target.value)} required className={inputStyles} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                   <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">Birds Vaccinated</label>
                   ...
                   <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">Next Due Date</label>
            ...
            {recordType === RecordType.PoultryCountChange && (
                 <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-600 mb-1">Change Type</label>
                     ...
                     <div>
                        <label className="block text-sm font-medium text-slate-600 mb-1">Number of Insects</label>
          ...
          <div className="bg-slate-50 px-6 py-4 flex justify-end space-x-3 border-t border-slate-200 rounded-b-lg">
            ...
            <button type="submit" className="px-4 py-2 bg-green-600 text-white border-transparent rounded-lg hover:bg-green-700 text-sm font-semibold">Save Record</button>
          </div>
        </form>
    </Modal>
  );
